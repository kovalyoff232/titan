...
fn test_analyze_calculates_distinct_values() {
    ...
    for row in stats {
        ...
        if col_num == "0" { // Assuming id is the first column (attnum 0)
            id_distinct = distinct_count.clone();
        } else if col_num == "1" { // Assuming category is the second column (attnum 1)
            category_distinct = distinct_count.clone();
        }
    }

    assert_eq!(id_distinct, "6", "Incorrect distinct count for 'id'");
    assert_eq!(category_distinct, "3", "Incorrect distinct count for 'category'");
}
